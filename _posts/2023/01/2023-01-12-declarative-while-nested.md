...
The previous [post](https://news.eolang.org/2022-12-22-declarative-while.html)
by @yegor256 explained the declarative WHILE nature.
Let's examine a more complex example when `while` objects are nested.
...
The outer loop continues while `x` is lower than `10`. The inner loop executes its body
...
Every time the inner `while` is dataized, `x` is incremented by `3`: twice through
`while` iterations and one more time once the body is returned as a result. So a single
