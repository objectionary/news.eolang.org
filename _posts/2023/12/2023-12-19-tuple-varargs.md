...
[0.34.1](https://github.com/objectionary/eo/releases/tag/0.34.1) of EO, and today we talk about the new
recursive implementation of the `tuple` object and why we got rid of varargs in our language.
...
stored an array of Java `Phi` objects, which was totally wrong.

But the decision was made - only `bytes` should have a `Δ` attribute, so we needed to remove it from
the other 5 objects including `tuple`. And it was a good chance to rewrite `tuple` in pure EO which
allowed us to decrease the number of [atoms](https://news.eolang.org/2022-12-02-java-atoms.html).
...
As you may see, `tuple` has two free attributes now: `head` and `tail`; where `head` is always
...
The `tuple.empty` is a special `tuple` that stores nothing and is used as a top-level `tuple` in the whole
recursion.
...
example, the `int.plus` object looked something like this:
...
It was quite a convenient mechanism, but it didn't match φ-calculus where every attribute has to
be attached to the concrete object. With varargs, the application `5.plus 5 10` looks something like
this:
...
So as not to complicate things, we decided to get rid of varargs in EO altogether. And now it's a perfect
